...
@Component({
  selector: 'jhi-them-sua-khach-hang',
  templateUrl: './them-sua-khach-hang.component.html',
  styleUrls: ['./them-sua-khach-hang.component.scss']
})
export class ThemSuaKhachHangComponent implements OnInit {
  ...
  onSubmit(typeSubmit?: any) {
    ...
    if (this.type === "add") {
      this.ThongTinApi
          .createKhachHang(data).subscribe(
          res => {
            this.spinner.hide();
    ...
    if (this.type === "update") {
      ...
      this.ThongTinApi
          .updateKhachHang(data).subscribe(
          res => {
            this.spinner.hide();
            this.toastService.openSuccessToast(
                "Cập nhập thành công"

...
import {ThemSuaChiNhanhComponent} from "app/modules/qlkh-manager/danh-muc/chi-nhanh-cua-hang/them-sua-chi-nhanh/them-sua-chi-nhanh.component";

@Component({
    selector: 'jhi-chi-nhanh-cua-hang',
    templateUrl: './chi-nhanh-cua-hang.component.html',
    styleUrls: ['./chi-nhanh-cua-hang.component.scss']
})
export class ChiNhanhCuaHangComponent implements OnInit {
    ...
    openModal(type?: string, selectedData?: any) {
        const modalRef = this.modalService.open(ThemSuaChiNhanhComponent, {
            size: "lg",
            backdrop: "static",
            keyboard: false
        });
    ...
    onSubmitDelete(id: any = []) {
        this.spinner.show();
        this.thongTinChungApiService.deleteChiNhanh({id: id}).subscribe(
            res => {
    ...
    // getListRole() {
    //   this.departmentManagementService.getRole().subscribe(
    //       res => {
    //         this.listRole = res.body
    //       },
    //       err => {
    //         this.toastService.openErrorToast(
    //             this.translateService.instant("common.toastr.messages.error.load")

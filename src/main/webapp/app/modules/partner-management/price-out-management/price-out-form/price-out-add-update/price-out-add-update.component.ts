...
@Component({
    selector: 'jhi-price-out-add-update',
    templateUrl: './price-out-add-update.component.html',
    styleUrls: ['./price-out-add-update.component.scss']
})
export class PriceOutAddUpdateComponent implements OnInit, OnDestroy {
    ...
    patchValueForm() {
        ...
        if (!this.isSaving && this.currentPriceOut) {
            console.log('this.currentPriceOut', this.currentPriceOut);
            this.formGroup.patchValue(this.currentPriceOut);
            arrDateControl.forEach(control => {
                this.formGroup.get(control).setValue(moment(this.currentPriceOut[control]).toDate());
            });
        }

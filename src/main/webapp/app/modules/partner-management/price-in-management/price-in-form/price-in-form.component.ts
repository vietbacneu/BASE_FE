...
import {PriceInAddUpdateComponent} from "app/modules/partner-management/price-in-management/price-in-form/price-in-add-update/price-in-add-update.component";

@Component({
    selector: 'jhi-price-in-form',
    templateUrl: './price-in-form.component.html',
    styleUrls: ['./price-in-form.component.scss']
})
export class PriceInFormComponent implements OnInit, OnDestroy {
    ...
    onSearch(pageable?: any) {
        ...
        this.partnerService.searchInComingPrice(rawValues, pageable).subscribe((next: any) => {
            console.log('next', next);
            this.lstPriceIn = next.content;
    ...
    openPriceInForm(isSaving: boolean, row?: any) {
        const dialogRef = this.modalService.open(PriceInAddUpdateComponent, {
            size: "lg",
            backdrop: "static",
            centered: true,
        });
        ...
        dialogRef.componentInstance.currentPriceIn = row;
        ...
        dialogRef.componentInstance.onClose.subscribe(value => {
            ...
            if (value) {
                this.saveInComingPrice(value, dialogRef);
            }
        });
    }
    ...
    doApprove(inComingPrice: any) {
        this.partnerService.updateInComingPrice(inComingPrice).pipe(finalize(() => this.hideSpinner())).subscribe((next: any) => {
    ...
    saveInComingPrice(inComingPrice: any, dialogRef: any) {
        this.showSpinner();
        if (!inComingPrice.id) {
            this.partnerService.insertInComingPrice(inComingPrice).pipe(finalize(() => this.hideSpinner())).subscribe((next: any) => {
                console.log('next add price', next);
                this.onSaveSuccess(this.translateService.instant('partner.priceIn').toLowerCase());
            ...
            this.partnerService.updateInComingPrice(inComingPrice).pipe(finalize(() => this.hideSpinner())).subscribe((next: any) => {
                ...
                this.onUpdateSuccess(this.translateService.instant('partner.priceIn').toLowerCase());
                dialogRef.componentInstance.onCancel();
                this.loadPage(1);
            }, (err: any) => {
                if (err && err.error.message === "existedDestination") {
    ...
    onDelete(item: any) {
        ...
        modalRef.componentInstance.onCloseModal.subscribe(value => {
            console.log(value);
            if (value) {
                this.partnerService.deleteInComingPrice(item.id).subscribe(() => {
                    this.onDeleteSuccess(this.translateService.instant('partner.priceIn').toLowerCase());
                    this.onSearch();
                }, () => this.showUnknownErr());
            }
        });

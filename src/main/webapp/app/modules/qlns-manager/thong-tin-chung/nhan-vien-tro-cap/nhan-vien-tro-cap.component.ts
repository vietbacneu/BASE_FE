...
import {ThemSuaNvTroCapComponent} from "app/modules/qlns-manager/thong-tin-chung/nhan-vien-tro-cap/them-sua-nv-tro-cap/them-sua-nv-tro-cap.component";
import * as moment from "moment";

@Component({
  selector: 'jhi-nhan-vien-tro-cap',
  templateUrl: './nhan-vien-tro-cap.component.html',
  styleUrls: ['./nhan-vien-tro-cap.component.scss']
})
export class NhanVienTroCapComponent implements OnInit {
  ...
  openModal(type?: string, selectedData?: any) {
    const modalRef = this.modalService.open(ThemSuaNvTroCapComponent, {
      size: "lg",
      backdrop: "static",
      keyboard: false
    });
  ...
  loadAll() {
    ...
    this.thongTinNhanSuApiService
        .searchNVTroCap({
          ...
          denNgay: this.form.value.toDate,
          tuNgay: this.form.value.fromDate,
          page: this.page - 1,
  ...
  onSubmitDelete(id: any = []) {
    this.spinner.show();
    this.thongTinNhanSuApiService.deleteNVTroCap({id: id}).subscribe(
        res => {

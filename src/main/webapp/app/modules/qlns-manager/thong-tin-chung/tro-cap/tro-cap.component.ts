...
import {ThemSuaTroCapComponent} from "app/modules/qlns-manager/thong-tin-chung/tro-cap/them-sua-tro-cap/them-sua-tro-cap.component";

@Component({
  selector: 'jhi-tro-cap',
  templateUrl: './tro-cap.component.html',
  styleUrls: ['./tro-cap.component.scss']
})
export class TroCapComponent implements OnInit {
  ...
  openModal(type?: string, selectedData?: any) {
    const modalRef = this.modalService.open(ThemSuaTroCapComponent, {
      size: "lg",
      backdrop: "static",
      keyboard: false
    });
  ...
  loadAll() {
    ...
    this.thongTinNhanSuApiService
        .searchDMTroCap({
          ...
          maTroCap: this.form.value.maTroCap,
          page: this.page - 1,
  ...
  onSubmitDelete(id: any = []) {
    this.spinner.show();
    this.thongTinNhanSuApiService.deleteDMTroCap({id: id}).subscribe(
        res => {

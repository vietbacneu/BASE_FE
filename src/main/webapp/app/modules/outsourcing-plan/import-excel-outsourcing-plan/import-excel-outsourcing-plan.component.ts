...
@Component({
  ...
  templateUrl: "./import-excel-outsourcing-plan.component.html",
  styleUrls: ["./import-excel-outsourcing-plan.component.scss"]
})
export class ImportExcelOutsourcingPlanComponent implements OnInit {
  ...
  @ViewChild("downloadTemplateBtn2", { static: false })
  downloadTemplateBtn2: ElementRef;
  ...
  onClickedOutside() {
    if (
      ...
      document.activeElement !== this.downloadTemplateBtn2.nativeElement &&
  ...
  downloadFileTemplate(excelType) {
    ...
    if ("xlsx" === excelType) {
      this.outsourcingPlanService
        .downloadFileExcel("templateOutsour.xlsx")
        .subscribe(res => {
          this.spinner.hide();
          if (res) {
      ...
      this.outsourcingPlanService
        .downloadFileExcel(
          "template_import_ke_hoach_thue_ngoai_phat_trien_pm.xls"
        )
        .subscribe(res => {
          this.spinner.hide();
          if (res) {
  ...
  onSubmitImport() {
    ...
    if (!this.file) {
      ...
      this.spinner.show();
      this.outsourcingPlanService.doImport(this.file).subscribe(
        res => {
          this.spinner.hide();
          if (res) {
            ...
            if (this.path === "errHeader") {
              ...
              this.eventManager.broadcast({ name: "outSourcingPlan" });
            }
          }
        },
        err => {
